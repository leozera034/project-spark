...
export function DeliveryPricingModePanel({ storeId, canEdit }: { storeId: string; canEdit: boolean }) {
  ...
  const query = useQuery({
    queryKey: ["delivery-pricing", storeId],
    queryFn: () => fetchDeliveryPricingConfig(storeId),
  ...
  const saveConfig = useMutation({
    ...
    onSuccess: async () => {
      toast.success("Modelo de taxa de entrega salvo.");
      await queryClient.invalidateQueries({ queryKey: ["delivery-pricing", storeId] });
    },
  ...
  const saveBands = useMutation({
    ...
    onSuccess: async () => {
      toast.success("Faixas por raio salvas.");
      await queryClient.invalidateQueries({ queryKey: ["delivery-pricing", storeId] });
    },
    onError: (error) => toast.error(error instanceof Error ? error.message : "Não foi possível salvar as faixas."),
  });

  if (query.isLoading) return <Card><CardContent className="p-6 text-sm text-muted-foreground">Carregando modelo de entrega…</CardContent></Card>;
  if (query.isError) return <Card><CardContent className="p-6 text-sm text-destructive">Não foi possível carregar o modelo de taxa de entrega.</CardContent></Card>;

  const choices: Array<{ value: DeliveryPricingMode; title: string; description: string; icon: typeof MapPinned }> = [
    { value: "fixed", title: "Taxa fixa na cidade", description: "Uma única taxa para todos os endereços atendidos.", icon: CircleDollarSign },
    { value: "radius", title: "Por distância / raio", description: "Crie faixas como até 3 km, 5 km, 8 km e cobre automaticamente.", icon: Radar },
    { value: "neighborhood", title: "Por bairro", description: "Mantém taxas específicas por bairro quando isso fizer sentido.", icon: MapPinned },
  ];
