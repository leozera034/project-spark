...
import { fulfillmentValidateSchema } from "@/lib/fulfillment-contracts";
...
export const Route = createFileRoute("/api/public/storefront/$slug/atendimento/validar")({
  server: {
    handlers: {
      ...
      POST: async ({ params, request }) => {
        ...
        try {
          ...
          const parsed = fulfillmentValidateSchema.parse({
            ...(JSON.parse(body || "{}") as Record<string, unknown>),
            slug: params.slug,
          });

          const mod = await import("@/lib/fulfillment.server");
          return json(await mod.validatePublicFulfillment(parsed));
        } catch (error) {
          if (error instanceof storefront.StorefrontError) {
            return json({ error: error.code }, error.code === "not_found" ? 404 : 503);
          }
          console.error("[api/public/storefront/atendimento/validar] failed");
