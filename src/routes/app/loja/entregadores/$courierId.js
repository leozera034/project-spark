...
import { useCourierDetail, useUpdateCourier, useActivateCourier, useDeactivateCourier, useResetCourierAccess } from "@/store/couriers/hooks/useCouriers";
import { useAuth } from "@/auth/useAuth";
import { Button } from "@/components/ui/button";
import { ChevronLeft, Calendar, Activity, ShieldCheck, ShieldAlert, History, Lock, Ban, CheckCircle, Copy, Check } from "lucide-react";
...
function CourierDetailPage() {
    ...
    if (isLoading) {
        return (<main className="container mx-auto max-w-4xl px-4 py-8 space-y-6">
    ...
    if (isError || !courier) {
        ...
        <ErrorState title="Entregador não encontrado" description="O perfil pode ter sido removido ou você não tem acesso a ele." onRetry={() => refetch()}/>
      </main>);
    }
    ...
    const handleResetAccess = async () => {
        if (!confirm("Isso invalidará a senha atual e gerará uma nova temporária. Continuar?"))
            return;
        try {
    ...
    return (<main className="container mx-auto max-w-5xl px-4 py-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          {courier.isActive ? (<Button variant="outline" className="text-destructive border-destructive/20 hover:bg-destructive/10" onClick={() => deactivateCourier.mutate({ storeId, courierId, expectedVersion: courier.version })} disabled={deactivateCourier.isPending}>
              <Ban className="mr-2 h-4 w-4"/>
              Inativar Conta
            </Button>) : (<Button variant="brand" onClick={() => activateCourier.mutate({ storeId, courierId, expectedVersion: courier.version })} disabled={activateCourier.isPending}>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <Card>
            ...
            <CardContent className="p-0">
              {courier.history.length === 0 ? (<div className="p-8 text-center text-muted-foreground italic">
                  Nenhuma atividade registrada ainda.
                </div>) : (<div className="divide-y border-t">
                  {courier.history.map((entry, idx) => (<div key={idx} className="flex items-start gap-4 p-4 text-sm">
                      ...
                      {entry.reasonCode && (<Badge variant="outline" className="text-[10px] uppercase">
                          {entry.reasonCode}
                        </Badge>)}
                    </div>))}
                </div>)}
        ...
        <div className="space-y-6">
          <Card>
            <CardHeader>
              <CardTitle className="text-sm uppercase tracking-wider text-muted-foreground">Status Atual</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex items-center gap-3">
                <div className={cn("h-3 w-3 rounded-full animate-pulse", courier.presenceStatus === "online" ? "bg-emerald-500" : "bg-muted-foreground/30")}/>
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span>{courier.lastSeenAt ? new Date(courier.lastSeenAt).toLocaleTimeString() : "Nunca"}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-muted-foreground flex items-center gap-1.5">
              ...
              <div className="space-y-2">
                ...
                {courier.currentAssignment ? (<div className="rounded-lg border bg-brand/5 border-brand/20 p-3">
                  ...
                  </div>) : (<p className="text-sm text-muted-foreground italic">Nenhuma entrega em curso.</p>)}
              </div>
            </CardContent>
          </Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                <div className="flex items-center justify-between text-xs">
                  ...
                  {courier.loginEnabled ? (<span className="text-emerald-600 flex items-center gap-1 font-medium"><ShieldCheck className="h-3 w-3"/> SIM</span>) : (<span className="text-destructive flex items-center gap-1 font-medium"><ShieldAlert className="h-3 w-3"/> NÃO</span>)}
                </div>
                <div className="flex items-center justify-between text-xs">
                  <span className="text-muted-foreground">Troca de Senha Pendente:</span>
                  {courier.requiresPasswordChange ? (<span className="text-amber-600 font-bold uppercase">Sim</span>) : (<span>Não</span>)}
                </div>
              </div>

              {resetResult ? (<div className="rounded border border-amber-300 bg-amber-100 p-3 dark:border-amber-800 dark:bg-amber-900/50">
                ...
                </div>) : (<Button variant="outline" size="sm" className="w-full text-xs" onClick={handleResetAccess} disabled={resetAccess.isPending}>
