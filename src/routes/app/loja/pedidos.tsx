...
export const Route = createFileRoute("/app/loja/pedidos")({
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Pedidos | Comandiva" },
...
function OrdersPanel() {
  ...
  const filters = useMemo(() => ({ statuses: queue.statuses, fulfillment, search, delayedOnly }), [queue.statuses, fulfillment, search, delayedOnly]);
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 lg:px-8">
      ...
      <header className="flex flex-wrap items-center justify-between gap-3">
        <div><h1 className="text-2xl font-semibold tracking-tight">Pedidos</h1><p className="mt-1 text-sm text-muted-foreground">Filas operacionais da sua loja, atualizadas automaticamente.</p></div>
        <div className="flex items-center gap-2">
          <Badge variant={live ? "default" : "secondary"} className="gap-1.5"><RadioTower className="size-3.5" />{live ? "Tempo real ativo" : "Atualização periódica"}</Badge>
          <Button variant="outline" size="icon" aria-label="Atualizar filas" onClick={() => { void countsQuery.refetch(); void listQuery.refetch(); }}><RefreshCw className="size-4" /></Button>
        </div>
      </header>
      ...
      {selectionRequired ? (
        ...
        <>
          <nav className="mt-6 flex flex-wrap gap-2" aria-label="Filas de pedidos">
            {ORDER_QUEUES.map((item) => <Button key={item.key} size="sm" variant={item.key === queueKey ? "default" : "outline"} onClick={() => setQueueKey(item.key)} aria-current={item.key === queueKey ? "page" : undefined}>{item.label}<span className="ml-1.5 tabular-nums opacity-70">{countFor(item.statuses)}</span></Button>)}
          </nav>
          <div className="mt-4 flex flex-wrap items-center gap-2">
            <div className="relative min-w-56 flex-1"><Search className="pointer-events-none absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Número do pedido ou nome" aria-label="Buscar pedidos" className="pl-9" /></div>
          ...
          <section className="mt-5">
            {listQuery.isLoading ? <div className="space-y-3"><Skeleton className="h-28 w-full rounded-xl" /><Skeleton className="h-28 w-full rounded-xl" /></div> : listQuery.error ? <ErrorState title="Não foi possível carregar a fila" description="Verifique sua conexão e tente de novo." onRetry={() => void listQuery.refetch()} /> : (listQuery.data?.orders.length ?? 0) === 0 ? <EmptyState title="Nenhum pedido nesta fila" description="Assim que um pedido chegar nesta etapa, ele aparece aqui automaticamente." /> : (
...
function OrderCard({ order, storeId, onOpen }: { order: StoreOrderListItem; storeId: string | null; onOpen: () => void }) {
  return (
    <li className="panel p-4">
      ...
      <div className="mt-4 flex flex-wrap gap-2"><OrderActions storeId={storeId} order={order} /><Button size="sm" variant="ghost" onClick={onOpen}>Detalhes</Button></div>
    </li>
  );
}
...
function OrderActions({ storeId, order }: { storeId: string | null; order: { id: string; version: number; allowedActions: StoreOrderAction[] } }) {
  ...
  return (
    <>
      {direct.map((action) => <Button key={action} size="sm" disabled={isRunning} onClick={() => void run({ action, orderId: order.id, expectedVersion: order.version }, `${ACTION_LABEL[action]}: feito.`)}>{ACTION_LABEL[action]}</Button>)}
      {withReason.map((action) => <Button key={action} size="sm" variant="outline" disabled={isRunning} onClick={() => setPendingAction(action)}>{ACTION_LABEL[action]}</Button>)}
...
function ReasonDialog({ action, storeId, orderId, version, onClose }: { action: StoreOrderAction | null; storeId: string | null; orderId: string; version: number; onClose: () => void }) {
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => (next ? null : close())}>
      <DialogContent>
        <DialogHeader><DialogTitle>{action === "reject" ? "Recusar pedido" : "Cancelar pedido"}</DialogTitle><DialogDescription>Escolha o motivo. O cliente vê apenas a mensagem pública ligada a ele; a observação interna fica só no histórico da loja.</DialogDescription></DialogHeader>
  ...
  return (
    <Sheet open={Boolean(orderId)} onOpenChange={(next) => (next ? null : onClose())}>
      <SheetContent side="right" className="flex w-full flex-col gap-0 overflow-y-auto p-0 sm:max-w-lg">
        <SheetHeader className="border-b border-border px-6 py-4 text-left"><SheetTitle>{detail ? `Pedido #${detail.orderNumber}` : "Detalhes do pedido"}</SheetTitle><SheetDescription>{detail ? `${STATUS_LABEL[detail.status]} · ${detail.fulfillment === "entrega" ? "Entrega" : "Retirada"}` : "Carregando informações do pedido."}</SheetDescription></SheetHeader>
        <div className="flex-1 overflow-y-auto px-6 py-4">
          {detailQuery.isLoading ? <Skeleton className="h-48 w-full rounded-xl" /> : detail ? (
            <div className="space-y-4 text-sm">
              ...
              {detail.delivery?.route ? <div className="rounded-xl border border-border bg-muted/40 p-3"><p className="flex items-center gap-2 font-semibold"><RouteIcon className="size-4" />{formatRouteDistance(detail.delivery.route.distanceMeters)} · {formatRouteDuration(detail.delivery.route.durationSeconds)}</p><p className="mt-1 text-xs text-muted-foreground">{routeQualityLabel(detail.delivery.route)} · tempo apenas de deslocamento.</p></div> : null}
              <Separator />
              ...
              <div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => printOrderReceipt(detail, { name: storeName })}><Printer className="size-4" /> Imprimir cupom (80mm)</Button><OrderActions storeId={storeId} order={{ id: detail.id, version: detail.version, allowedActions: detail.allowedActions }} /></div>
              {detail.fulfillment === "entrega" ? <DeliveryAssignmentPanel storeId={storeId} orderId={detail.id} /> : null}
              <Separator />
              <div><h3 className="text-sm font-semibold">Histórico</h3>{historyQuery.isLoading ? <Skeleton className="mt-2 h-20 w-full rounded-lg" /> : historyQuery.error ? <p className="mt-2 text-xs text-muted-foreground">Seu perfil não tem acesso ao histórico completo deste pedido.</p> : <ol className="mt-2 space-y-2">{(historyQuery.data ?? []).map((entry, index) => <li key={`${entry.occurredAt}-${index}`} className="text-xs"><span className="font-medium">{STATUS_LABEL[entry.toStatus]}</span>{" "}<span className="text-muted-foreground">{new Date(entry.occurredAt).toLocaleString("pt-BR")} · {entry.actorName ?? entry.actorKind}</span>{entry.internalNote ? <p className="text-muted-foreground">{entry.internalNote}</p> : null}</li>)}</ol>}</div>
            </div>
...
function DeliveryAssignmentPanel({ storeId, orderId }: { storeId: string | null; orderId: string }) {
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border p-4">
      <div><h3 className="font-semibold">Entregador e rota</h3><p className="text-xs text-muted-foreground">{current ? `Atribuído a ${current.displayName} · ${COURIER_VEHICLE_LABEL[current.vehicle]}` : "Escolha quem fará esta entrega. O veículo define o modo de rota."}</p></div>
      {route ? <div className="rounded-lg bg-muted/50 p-3 text-sm"><p className="font-semibold">{formatRouteDistance(route.distanceMeters)} · {formatRouteDuration(route.durationSeconds)}</p><p className="text-xs text-muted-foreground">{routeQualityLabel(route)} · deslocamento, não tempo total do pedido.</p></div> : null}
      {eligibleQuery.isLoading ? <Skeleton className="h-10 w-full rounded-md" /> : (eligibleQuery.data?.length ?? 0) === 0 ? <p className="text-sm text-muted-foreground">Nenhum entregador cadastrado.</p> : (
        <div className="grid gap-2 sm:grid-cols-2">
          {eligibleQuery.data?.map((courier) => <Button key={courier.courierId} type="button" variant={current?.courierId === courier.courierId ? "default" : "outline"} className="h-auto min-h-11 flex-col items-start gap-0.5 py-2 text-left" disabled={assign.isPending || courier.eligibility === "blocked" || Boolean(current)} onClick={() => assign.mutate({ storeId, orderId, courierId: courier.courierId, expectedDeliveryVersion: assignment.delivery?.version ?? 0 })}><span>{courier.displayName} · {COURIER_VEHICLE_LABEL[courier.vehicle]}</span><span className="text-[10px] font-normal opacity-70">{courier.blockingReason ? BLOCKING_REASON_LABEL[courier.blockingReason] ?? courier.blockingReason : courier.presenceStatus === "offline" ? "Offline — confirme antes" : "Disponível"}</span></Button>)}
        </div>
      )}
    </section>
  );
