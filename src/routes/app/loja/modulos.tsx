...
function StoreResourcesPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
      <header>
        <p className="text-xs font-extrabold uppercase tracking-[.14em] text-brand">Recursos da loja</p>
      ...
      <Card className="overflow-hidden border-success/25">
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div className="flex min-w-0 items-start gap-3">
              ...
              <div>
                ...
                <CardDescription className="mt-1">Avisos automáticos de pedido, templates personalizados e envio manual pelo mesmo número conectado.</CardDescription>
              </div>
            </div>
        ...
        <CardContent className="space-y-4 p-5">
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
            {["Conexão por QR Code", "Avisos por status do pedido", "Templates editáveis", "Histórico de mensagens"].map((item) => <div key={item} className="flex items-center gap-2 rounded-xl border border-border bg-surface-muted/30 p-3 text-sm"><CheckCircle2 className="size-4 shrink-0 text-success" /> {item}</div>)}
          </div>
