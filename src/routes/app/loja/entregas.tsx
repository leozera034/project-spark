...
import { useStoreSmartDeliveryControlCenter } from "@/store/integrations/store-smart-delivery.queries";
...
function DeliveriesHub() {
  ...
  const smartAvailable = Boolean(
    smart?.smart_delivery_entitled &&
      smart.capabilities.routes_ready &&
      !smart.control.is_paused,
  );
    ...
    <div className="mx-auto w-full max-w-6xl space-y-6 px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-sm leading-6 text-muted-foreground">Equipe, pedidos aguardando saída, devoluções e regras de entrega reunidos em uma única central.</p>
        </div>
      ...
      {smart?.smart_delivery_entitled ? (
        <Card className={smartAvailable ? "border-success/25" : "border-warning/30"}>
          <CardHeader className="pb-3">
            <div className="flex items-start gap-3">
              <span className={`grid size-10 shrink-0 place-items-center rounded-xl ${smartAvailable ? "bg-success-soft text-success" : "bg-warning-soft text-warning"}`}><Truck className="size-5" /></span>
              <div>
                <div className="flex flex-wrap items-center gap-2"><CardTitle>Estimativa inteligente de entrega</CardTitle><Badge variant={smartAvailable ? "success" : "warning"}>{smartAvailable ? "Disponível" : "Disponibilidade reduzida"}</Badge></div>
                <CardDescription className="mt-1">A Comandiva usa a melhor estimativa disponível para distância e prazo sem interromper seus pedidos.</CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            {!smart.store.coordinates_set ? (
              <div className="flex flex-col gap-3 rounded-xl border border-warning/25 bg-warning-soft/40 p-4 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-sm text-muted-foreground">Confirme o endereço da loja para melhorar a estimativa de distância.</p>
                <Button asChild variant="outline" size="sm"><Link to="/app/loja/configuracoes/endereco">Conferir endereço</Link></Button>
              </div>
            ) : (
              <p className="text-sm text-muted-foreground">Endereço da loja confirmado. As estimativas são aplicadas automaticamente quando disponíveis.</p>
            )}
          </CardContent>
        </Card>
      ) : null}
      ...
      <Card className="border-brand/15 bg-brand-soft/25">
        <CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-start gap-3"><span className="grid size-10 shrink-0 place-items-center rounded-xl bg-background text-brand"><MapPin className="size-5" /></span><div><p className="font-bold">Endereço da loja</p><p className="mt-1 text-sm text-muted-foreground">Mantenha o endereço correto para melhorar distância, taxa e previsão de entrega.</p></div></div>
