...
const RELEVANT_ROLES = new Set(["cream", "fruit", "topping", "addon", "protein", "side", "sauce", "container"]);
  ...
  return (
    <div className="grid gap-3 rounded-2xl border border-border bg-background/25 p-3 sm:grid-cols-[1fr_130px_130px_auto] sm:items-end">
      <div>
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">Padrão atual: até {group.max_selections} · {group.included_selections ?? 0} incluídos</p>
      </div>
      <div className="space-y-1.5">
        <Label>Até quantos</Label>
  ...
  return (
    <Card className="overflow-hidden border-violet-400/15 bg-[radial-gradient(circle_at_top_right,rgba(99,102,241,.09),transparent_42%),var(--card)]">
      <CardHeader>
        ...
        <CardTitle className="text-base">O que muda por tamanho?</CardTitle>
        <CardDescription>
        ...
        {variants.map((variant) => (
          <section key={variant.id} className="space-y-2 rounded-2xl border border-border/70 p-3">
            <div className="flex items-center justify-between gap-3">
              <div><p className="font-bold">{variant.name}</p><p className="text-xs text-muted-foreground">Ajustes exclusivos deste tamanho</p></div>
