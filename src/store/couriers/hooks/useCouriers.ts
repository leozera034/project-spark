...
export function useCourierCounts(storeId: string | null) {
  return useQuery({ queryKey: ["couriers", "counts", storeId], queryFn: () => fetchCourierCounts(storeId), enabled: !!storeId });
}
...
export function useCreateCourier() {
  ...
  return useMutation({
    mutationFn: createFn,
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["couriers"] }); toast.success("Entregador cadastrado com sucesso!"); },
...
export function useUpdateCourier() {
  ...
  return useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["couriers"] }); toast.success("Perfil atualizado!"); },
    onError: (error) => {
...
export function useActivateCourier() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: activateCourier, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["couriers"] }); toast.success("Entregador ativado."); } });
}

export function useDeactivateCourier() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: deactivateCourier, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["couriers"] }); toast.success("Entregador inativado."); } });
}
...
export function useResetCourierAccess() {
  ...
  return useMutation({
    ...
    onSuccess: () => toast.success("Acesso redefinido com sucesso."),
    onError: (error) => toast.error(toFriendlyMessage(error)),
  });
}

export function useEligibleCouriers(storeId: string | null, orderId: string) {
  return useQuery({ queryKey: ["couriers", "eligible", storeId, orderId], queryFn: () => fetchEligibleCouriers(storeId, orderId), enabled: !!storeId && !!orderId });
}

export function useDeliveryAssignment(storeId: string | null, orderId: string) {
  return useQuery({ queryKey: ["couriers", "assignment", storeId, orderId], queryFn: () => fetchDeliveryAssignment(storeId, orderId), enabled: Boolean(storeId && orderId) });
}

export function useAssignCourier() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: assignCourier, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["store-orders"] }); queryClient.invalidateQueries({ queryKey: ["couriers"] }); toast.success("Entregador atribuído."); } });
}

export function useReassignCourier() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: reassignCourier, onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["store-orders"] }); queryClient.invalidateQueries({ queryKey: ["couriers"] }); toast.success("Entregador trocado."); } });
}

export function useMyCourierOperationalContext() {
  return useQuery({ queryKey: ["courier", "operational-context"], queryFn: () => fetchMyCourierOperationalContext(), staleTime: 30000, refetchOnWindowFocus: true });
}
...
export function useSetCourierOnline() {
  ...
  return useMutation({
    ...
    onSuccess: (data: CourierPresenceResult) => {
      queryClient.setQueryData(["courier", "operational-context"], (old: any) => old ? { ...old, onlineIntent: data.onlineIntent, presenceStatus: data.presenceStatus, lastSeenAt: data.lastSeenAt, version: data.version } : old);
...
export function useCourierHeartbeat() {
  const queryClient = useQueryClient();
  return useMutation({ mutationFn: heartbeatMyCourierPresence, onSuccess: (data: CourierPresenceResult) => queryClient.setQueryData(["courier", "operational-context"], (old: any) => old ? { ...old, presenceStatus: data.presenceStatus, lastSeenAt: data.lastSeenAt, version: data.version } : old) });
}
