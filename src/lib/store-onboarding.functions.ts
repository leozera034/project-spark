 ...
 * externo. Um plano pago selecionado é apenas intenção comercial: o banco
 * provisiona o fallback gratuito e a Stripe confirma qualquer ativação paga.
 */
...
export const checkStoreSlug = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: result, error } = await supabaseAdmin.rpc("check_public_store_slug", { _slug: data.slug } as never);
    if (error) return { slug: data.slug, available: false, reason: "indisponivel" as string | null };
  ...
  .handler(async ({ data }) => {
    ...
    try {
      return await invokePediuBackendAction<{ storeId: string; slug: string }>({
        ...
        input: { ...data, email: data.email.toLowerCase(), state: data.state.toUpperCase() },
      });
    } catch (error) {
      if (error instanceof PediuBackendApiError) {
        switch (error.code) {
          case "slug_in_use": throw new Error("Esse endereço de loja já está em uso.");
          case "invalid_slug": throw new Error("Endereço de loja inválido.");
          case "email_in_use": throw new Error("Já existe uma conta com esse e-mail. Entre com ela ou use outro e-mail.");
          case "invalid_input": throw new Error("Revise os dados informados e tente novamente.");
          case "rate_limited": throw new Error("Muitas tentativas de cadastro. Aguarde alguns minutos e tente novamente.");
          case "account_creation_failed": throw new Error("Não foi possível criar o acesso do proprietário.");
          default: break;
        }
      }
