...
export const createStoreAccount=createServerFn({method:"POST"}).inputValidator((data:unknown)=>createSchema.parse(data)).handler(async({data})=>{
  ...
  try{
    ...
    if(error instanceof PediuBackendApiError){switch(error.code){case"slug_in_use":throw new Error("Esse endereço de loja já está em uso.");case"invalid_slug":throw new Error("Endereço de loja inválido.");case"email_in_use":throw new Error("Já existe uma conta com esse e-mail. Entre com ela ou use outro e-mail.");case"legal_not_accepted":throw new Error("Você precisa aceitar os Termos de Uso e a Política de Privacidade para criar a loja.");case"legal_audit_failed":throw new Error("Não foi possível registrar o aceite dos termos com segurança. Tente novamente.");case"invalid_input":throw new Error("Revise os dados informados e tente novamente.");case"rate_limited":throw new Error("Muitas tentativas de cadastro. Aguarde alguns minutos e tente novamente.");case"account_creation_failed":throw new Error("Não foi possível criar o acesso do proprietário.");case"provision_failed":throw new Error("O acesso foi criado, mas a loja não pôde ser provisionada com segurança.");default:break}}
