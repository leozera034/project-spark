...
export const checkoutAddressSchema = z.object({
  ...
  if (hasLatitude !== hasLongitude) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: "latitude e longitude devem ser enviadas juntas", path: [hasLatitude ? "longitude" : "latitude"] });
  }
});

const optionalPhoneSchema = z.string().trim().max(24).refine(
  (value) => value.length === 0 || normalizePhone(value) !== null,
  "telefone inválido",
);
