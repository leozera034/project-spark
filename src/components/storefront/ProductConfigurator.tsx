...
export function ProductConfigurator({ slug, productId, storeOpen, editLineId = null, onClose }: Props) {
  ...
  const explicitMeal = Boolean(data?.product.capabilities?.meal_experience);
  const isMealBuildable = !isBurgerBuildable && (explicitMeal || (rawBuildGroups.some((group) => group.role === "protein") && rawBuildGroups.some((group) => group.role === "side")));
