...
const BUILDABLE_ROLES = new Set(["cream", "fruit", "topping", "addon", "protein", "side", "sauce", "container", "bread", "doneness", "removal", "beverage"]);
const MEAL_ROLE_ORDER: Record<string, number> = { protein: 10, side: 20, addon: 30, removal: 40, beverage: 50 };
const BURGER_ROLE_ORDER: Record<string, number> = { bread: 10, protein: 20, doneness: 30, sauce: 40, addon: 50, removal: 60, side: 70, beverage: 80 };

function mealStepTitle(group: PublicOptionGroup) {
  if (group.role === "protein") return "Escolha sua proteína";
  if (group.role === "side") return "Monte os acompanhamentos";
  if (group.role === "addon") return "Quer adicionar algo?";
  if (group.role === "removal") return "Quer retirar algum ingrediente?";
  if (group.role === "beverage") return "Quer uma bebida?";
  return group.name;
}

function mealStepHint(group: PublicOptionGroup) {
  if (group.role === "protein") return "Escolha a proteína principal da sua refeição.";
  if (group.role === "side") return "Escolha os acompanhamentos permitidos para este tamanho.";
  if (group.role === "addon") return "Adicionais são opcionais e podem alterar o valor.";
  if (group.role === "removal") return "Opcional. Marque apenas o que você não quer receber.";
  if (group.role === "beverage") return "Opcional. Você pode seguir sem bebida.";
  return group.description ?? "Faça sua escolha para continuar.";
}

function burgerStepTitle(group: PublicOptionGroup) {
  if (group.role === "bread") return "Escolha o pão";
  if (group.role === "protein") return "Escolha o hambúrguer";
  if (group.role === "doneness") return "Como você quer a carne?";
  if (group.role === "sauce") return "Escolha os molhos";
  if (group.role === "addon") return "Quer turbinar seu lanche?";
  if (group.role === "removal") return "Quer retirar algum ingrediente?";
  if (group.role === "side") return "Quer um acompanhamento?";
  if (group.role === "beverage") return "Quer uma bebida?";
  return group.name;
}

function burgerStepHint(group: PublicOptionGroup) {
  if (group.role === "bread") return "Escolha a base do seu lanche.";
  if (group.role === "protein") return "Escolha a carne ou opção principal do hambúrguer.";
  if (group.role === "doneness") return "Quando aplicável, escolha o ponto da carne.";
  if (group.role === "sauce") return "Molhos podem ser combinados até o limite configurado.";
  if (group.role === "addon") return "Bacon, queijo e outros extras podem alterar o valor.";
  if (group.role === "removal") return "Opcional. Marque somente o que você não quer no lanche.";
  if (group.role === "side") return "Opcional. Escolha batata, onion rings ou outro acompanhamento disponível.";
  if (group.role === "beverage") return "Opcional. Você pode finalizar o lanche sem bebida.";
  return group.description ?? "Faça sua escolha para continuar.";
}
...
export function ProductConfigurator({ slug, productId, storeOpen, editLineId = null, onClose }: Props) {
  ...
  const rawBuildGroups = groups.filter((group) => BUILDABLE_ROLES.has(group.role) || group.included_selections > 0 || (data?.variant_group_rules ?? []).some((rule) => rule.option_group_id === group.id && (rule.included_selections ?? 0) > 0));
  const isBurgerBuildable = Boolean(data?.product.capabilities?.burger_experience);
  const isMealBuildable = !isBurgerBuildable && Boolean(data?.product.capabilities?.meal_experience);
  const buildGroups = isBurgerBuildable
    ? [...rawBuildGroups].sort((a, b) => (BURGER_ROLE_ORDER[a.role] ?? 100) - (BURGER_ROLE_ORDER[b.role] ?? 100))
    : isMealBuildable
      ? [...rawBuildGroups].sort((a, b) => (MEAL_ROLE_ORDER[a.role] ?? 100) - (MEAL_ROLE_ORDER[b.role] ?? 100))
      : rawBuildGroups;
  const isGuidedCombo = data?.product.product_type === "combo" && comboGroups.length > 0;
  const isGuidedBuildable = !isGuidedCombo && buildGroups.length >= 2 && (isBurgerBuildable || isMealBuildable || data?.product.product_type === "buildable" || Boolean(data?.product.capabilities?.included_choices) || buildGroups.some((group) => ["cream", "fruit", "topping"].includes(group.role)));
  ...
  const optionPriceFor = (itemId: string, fallback: number) => {
    if (!variantId) return fallback;
    const row = data?.variant_option_prices?.find((entry) => entry.product_variant_id === variantId && entry.option_item_id === itemId);
    return row ? row.price : fallback;
  };
  ...
  const changeVariant = (nextId: string) => {
    ...
    for (const selection of nonFlavorSelections) byGroup.set(selection.option_group_id, [...(byGroup.get(selection.option_group_id) ?? []), selection]);
  ...
  const chooseBuildSingle = (group: PublicOptionGroup, itemId: string, buildIndex: number) => {
    toggleSingle(group.id, itemId);
    if (buildIndex >= 0 && buildIndex < buildGroups.length - 1) window.setTimeout(() => setActiveBuildStep(buildIndex + 1), 120);
  };
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex-1 space-y-6 overflow-y-auto px-1 pb-6">
        ...
        {isGuidedBuildable ? <section className={`rounded-3xl border p-4 ${isBurgerBuildable ? "border-amber-400/20 bg-[radial-gradient(circle_at_top_right,rgba(245,158,11,.14),transparent_45%),rgba(10,8,22,.5)]" : "border-violet-400/20 bg-[radial-gradient(circle_at_top_right,rgba(59,130,246,.13),transparent_45%),rgba(10,8,22,.5)]"}`}><div className="flex items-start justify-between gap-3"><div><p className={`text-[10px] font-black uppercase tracking-[.16em] ${isBurgerBuildable ? "text-amber-300" : "text-violet-300"}`}>{isBurgerBuildable ? "Monte seu lanche" : isMealBuildable ? "Monte sua refeição" : "Sua montagem"}</p><h3 className="mt-1 text-base font-bold">Etapa {Math.min(activeBuildStep + 1, buildGroups.length)} de {buildGroups.length}</h3><p className="mt-1 text-xs text-muted-foreground">{isBurgerBuildable ? "Escolha pão, carne, molhos e extras. Etapas opcionais podem ser puladas." : isMealBuildable ? "Escolha proteína, acompanhamentos e extras. O tamanho define os limites automaticamente." : "Os limites se adaptam automaticamente ao tamanho escolhido."}</p></div><Badge variant="secondary">{buildCompleteCount}/{buildGroups.length}</Badge></div><div className="mt-4 flex gap-1.5">{buildGroups.map((group, index) => { const done = !pendingGroups.some((pending) => pending.id === group.id); return <button key={group.id} type="button" onClick={() => setActiveBuildStep(index)} className={`h-2 flex-1 rounded-full ${index === activeBuildStep ? isBurgerBuildable ? "bg-amber-400" : "bg-violet-400" : done ? "bg-emerald-400/70" : "bg-white/10"}`} />; })}</div></section> : null}

        {data.variants.length > 0 ? <section className="space-y-3 rounded-2xl border border-border/70 p-4"><header className="flex items-start gap-3"><span className="grid size-7 place-items-center rounded-full bg-violet-500/15 text-xs font-black text-violet-200">1</span><div><h3 className="text-sm font-semibold">{isMealBuildable ? "Escolha o tamanho da marmita" : isBurgerBuildable ? "Escolha o tamanho do lanche" : "Escolha o tamanho"}</h3><p className="text-xs text-muted-foreground">{isMealBuildable ? "Cada tamanho pode permitir uma quantidade diferente de acompanhamentos." : isBurgerBuildable ? "O tamanho pode alterar preço e limites da montagem." : "O tamanho pode alterar preço, limite e quantidade de itens incluídos."}</p></div></header><RadioGroup value={variantId ?? ""} onValueChange={changeVariant} className="space-y-2">{data.variants.map((variant) => <label key={variant.id} className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border p-3"><span className="flex items-center gap-3"><RadioGroupItem value={variant.id} id={variant.id} /><span className="text-sm font-medium">{variant.name}</span></span><span className="text-sm font-semibold">{brl(variant.price)}</span></label>)}</RadioGroup></section> : null}
        ...
        {groups.map((rawGroup) => {
          ...
          const buildTitle = isBurgerBuildable ? burgerStepTitle(group) : isMealBuildable ? mealStepTitle(group) : group.name;
          const buildHint = isBurgerBuildable ? burgerStepHint(group) : isMealBuildable ? mealStepHint(group) : groupInstruction(group, chosen, maxSelections);
          if (isBuildGroup && buildIndex !== activeBuildStep) return <button key={group.id} type="button" onClick={() => setActiveBuildStep(buildIndex)} className={`flex w-full items-center justify-between rounded-2xl border p-4 text-left ${!incomplete ? "border-emerald-400/15 bg-emerald-500/[.035]" : "border-border"}`}><div><p className="text-[10px] font-black uppercase text-muted-foreground">Etapa {buildIndex + 1}</p><p className="text-sm font-bold">{buildTitle}</p><p className="text-xs text-muted-foreground">{quota ?? (chosen ? `${chosen} escolhido${chosen > 1 ? "s" : ""}` : groupMinimum(group) > 0 ? "Falta escolher" : "Opcional")}</p></div>{!incomplete ? <CheckCircle2 className="size-5 text-emerald-300" /> : <ChevronRight className="size-5" />}</button>;

          return <section key={group.id} className={`space-y-3 rounded-3xl border p-4 ${isBuildGroup ? isBurgerBuildable ? "border-amber-400/20 bg-amber-500/[.025]" : "border-violet-400/20 bg-violet-500/[.035]" : "border-border/70"}`}><header className="flex items-start justify-between gap-3"><div><h3 className="text-sm font-semibold">{isBuildGroup ? buildTitle : group.name}</h3><p className="text-xs text-muted-foreground">{isBuildGroup ? buildHint : groupInstruction(group, chosen, maxSelections)}</p>{isBuildGroup && isMealBuildable && group.role === "side" ? <p className="mt-1 text-xs font-semibold text-violet-300">{selectedVariant?.name ? `${selectedVariant.name}: ` : ""}escolha até {maxSelections} acompanhamento{maxSelections > 1 ? "s" : ""}.</p> : null}{isBuildGroup && isBurgerBuildable && group.role === "side" && maxSelections > 0 ? <p className="mt-1 text-xs font-semibold text-amber-300">Escolha até {maxSelections} acompanhamento{maxSelections > 1 ? "s" : ""}.</p> : null}{quota ? <p className={`mt-1 text-xs font-bold ${chosen > group.included_selections ? "text-amber-300" : "text-emerald-300"}`}>{quota}</p> : null}</div>{incomplete ? <Badge variant="destructive">Falta escolher</Badge> : chosen > 0 ? <Badge variant="secondary">{chosen}/{maxSelections}</Badge> : groupMinimum(group) === 0 ? <Badge variant="outline">Opcional</Badge> : null}</header>{group.included_selections > 0 ? <div className="rounded-2xl border border-emerald-400/10 bg-emerald-500/[.03] p-3"><div className="flex justify-between text-xs font-semibold"><span>{isMealBuildable && group.role === "side" ? "Acompanhamentos incluídos" : `Incluídos no tamanho ${selectedVariant?.name ?? "escolhido"}`}</span><span>{Math.min(chosen, group.included_selections)}/{group.included_selections}</span></div><div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10"><div className="h-full rounded-full bg-emerald-400" style={{ width: `${Math.min(100, (chosen / Math.max(1, group.included_selections)) * 100)}%` }} /></div>{chosen > group.included_selections ? <p className="mt-2 text-xs font-semibold text-amber-300">{chosen - group.included_selections} escolha{chosen - group.included_selections > 1 ? "s" : ""} além do incluído. O servidor atualiza o total.</p> : null}</div> : null}<div className="grid gap-2 sm:grid-cols-2">{group.items.map((item) => { const selected = selections.find((selection) => selection.option_group_id === group.id && selection.option_item_id === item.id); const itemPrice = optionPriceFor(item.id, item.additional_price); const priceLabel = optionPriceLabel(group, itemPrice); if (group.selection_type === "quantidade" || group.allow_quantity) return <div key={item.id} className={`flex items-center justify-between rounded-2xl border p-3 ${selected ? "border-violet-400/25 bg-violet-500/[.04]" : "border-border"}`}><div><p className="text-sm font-semibold">{item.name}</p><p className="text-xs text-muted-foreground">{priceLabel ?? "Sem acréscimo"}</p></div><div className="flex items-center gap-2"><Button size="icon" variant="outline" disabled={!selected} onClick={() => bumpQuantity(group.id, item.id, -1, item.max_quantity, maxSelections)}><Minus className="size-4" /></Button><span>{selected?.quantity ?? 0}</span><Button size="icon" variant="outline" disabled={atLimit || (selected?.quantity ?? 0) >= item.max_quantity} onClick={() => bumpQuantity(group.id, item.id, 1, item.max_quantity, maxSelections)}><Plus className="size-4" /></Button></div></div>; if (group.selection_type === "unica") return <label key={item.id} className={`flex min-h-14 cursor-pointer items-center justify-between rounded-2xl border p-3 ${selected ? isBurgerBuildable ? "border-amber-400/40 bg-amber-500/[.07]" : "border-violet-400/40 bg-violet-500/[.07]" : "border-border"}`}><span className="text-sm font-semibold">{item.name}</span><input type="radio" className="sr-only" name={group.id} checked={Boolean(selected)} onChange={() => isBuildGroup ? chooseBuildSingle(group, item.id, buildIndex) : toggleSingle(group.id, item.id)} /></label>; return <label key={item.id} className={`flex min-h-14 cursor-pointer items-center justify-between rounded-2xl border p-3 ${selected ? isBurgerBuildable ? "border-amber-400/40 bg-amber-500/[.07]" : "border-violet-400/40 bg-violet-500/[.07]" : "border-border"}`}><span className="flex items-center gap-3"><Checkbox checked={Boolean(selected)} onCheckedChange={() => toggleMulti(group.id, item.id, maxSelections)} /><span className="text-sm font-semibold">{item.name}</span></span><span className="text-xs text-muted-foreground">{group.role === "removal" ? "Retirar" : priceLabel ?? "Dentro da montagem"}</span></label>; })}</div>{isBuildGroup ? <div className="flex items-center justify-between gap-2 border-t pt-3"><Button size="sm" variant="ghost" disabled={buildIndex === 0} onClick={() => setActiveBuildStep(buildIndex - 1)}><ChevronLeft className="mr-1 size-4" />Voltar</Button>{groupMinimum(group) === 0 && buildIndex < buildGroups.length - 1 ? <Button size="sm" variant="ghost" onClick={() => setActiveBuildStep(buildIndex + 1)}>Agora não</Button> : null}<Button size="sm" variant="outline" disabled={incomplete || buildIndex === buildGroups.length - 1} onClick={() => setActiveBuildStep(buildIndex + 1)}>Próxima<ChevronRight className="ml-1 size-4" /></Button></div> : null}</section>;
        })}

        {isGuidedBuildable && buildCompleteCount === buildGroups.length ? <section className="rounded-3xl border border-emerald-400/15 bg-emerald-500/[.035] p-4"><div className="flex gap-3"><CheckCircle2 className="size-5 text-emerald-300" /><div><h3 className="font-bold">{isBurgerBuildable ? "Seu lanche está pronto" : isMealBuildable ? "Sua marmita está pronta" : "Sua montagem está pronta"}</h3><p className="text-xs text-muted-foreground">Revise qualquer etapa antes de adicionar ao pedido.</p></div></div></section> : null}
        <Separator />
        ...
        {product.allows_notes ? <section className="space-y-2"><Label htmlFor="observacao">Observação</Label><Textarea id="observacao" value={notes} maxLength={280} onChange={(event) => setNotes(event.target.value)} placeholder={isBurgerBuildable ? "Ex.: molho separado" : isMealBuildable ? "Ex.: feijão separado" : "Ex.: sem cebola"} /></section> : null}
