...
export function ProductConfigurator({ slug, productId, storeOpen, editLineId = null, onClose }: Props) {
  ...
  const isMealBuildable = rawBuildGroups.some((group) => group.role === "protein") && rawBuildGroups.some((group) => group.role === "side");
  const buildGroups = isMealBuildable
  ...
  const isGuidedBuildable = !isGuidedCombo && buildGroups.length >= 2 && (isMealBuildable || data?.product.product_type === "buildable" || Boolean(data?.product.capabilities?.included_choices) || buildGroups.some((group) => ["cream", "fruit", "topping"].includes(group.role)));
    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 space-y-6 overflow-y-auto px-1 pb-6">
        ...
        {groups.map((rawGroup) => {
          ...
          const buildTitle = isMealBuildable ? mealStepTitle(group) : group.name;
          const buildHint = isMealBuildable ? mealStepHint(group) : groupInstruction(group, chosen, maxSelections);
        ...
        {product.allows_notes ? <section className="space-y-2"><Label htmlFor="observacao">Observação</Label><Textarea id="observacao" value={notes} maxLength={280} onChange={(event) => setNotes(event.target.value)} placeholder={isMealBuildable ? "Ex.: feijão separado" : "Ex.: sem cebola"} /></section> : null}
