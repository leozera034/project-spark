...
begin
  if nullif(btrim(coalesce(_payment_intent_id,'')),'') is null then
    raise exception 'STRIPE_PAYMENT_INTENT_REQUIRED' using errcode='P0001';
  end if;
  if nullif(btrim(coalesce(_stripe_account_id,'')),'') is null then
    raise exception 'STRIPE_ACCOUNT_REQUIRED' using errcode='P0001';
  end if;
  ...
  if _order.payment_method_kind is distinct from 'stripe_online' then
    raise exception 'ORDER_PAYMENT_METHOD_MISMATCH' using errcode='P0001';
  end if;

  _expected_amount:=round(_order.total_amount*100)::bigint;
  if _amount_cents is null or _amount_cents::bigint<>_expected_amount then
    raise exception 'ORDER_PAYMENT_AMOUNT_MISMATCH' using errcode='P0001';
  end if;
  if lower(coalesce(_currency,''))<>'brl' then
    raise exception 'ORDER_PAYMENT_CURRENCY_MISMATCH' using errcode='P0001';
  end if;
  if coalesce(_application_fee_amount,0)<0 or coalesce(_application_fee_amount,0)>_amount_cents then
    raise exception 'ORDER_PAYMENT_FEE_INVALID' using errcode='P0001';
  end if;

  select a.stripe_account_id into _mapped_account
  from private.stripe_connect_accounts a
  where a.store_id=_store_id;
  if _mapped_account is null then
    raise exception 'STRIPE_ACCOUNT_NOT_MAPPED' using errcode='P0001';
  end if;
  if _mapped_account is distinct from _stripe_account_id then
  ...
  if _pattern not in ('direct','separate') then
    raise exception 'STRIPE_CHARGE_PATTERN_INVALID' using errcode='P0001';
  end if;
  ...
  if _status='succeeded' then
    ...
    _journal_entries:=jsonb_build_array(
      jsonb_build_object(
        ...
        'direction','debit','component_type','payment_gross','amount_cents',_amount_cents
      ),
      jsonb_build_object(
        'account_code','store.sales.clearing','owner_type','store','direction','credit',
        'component_type','store_proceeds','amount_cents',_store_net
      )
    );
    if greatest(_application_fee_amount,0)>0 then
      _journal_entries:=_journal_entries||jsonb_build_array(
        jsonb_build_object(
          ...
          'component_type','platform_fee','amount_cents',greatest(_application_fee_amount,0)
        )
      );
    end if;
