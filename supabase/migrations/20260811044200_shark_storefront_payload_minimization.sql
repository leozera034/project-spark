-- SHARK — minimização do payload público do configurador.
-- O motor pode continuar usando configuration/metadata internamente, mas o browser
-- recebe somente os campos necessários para renderização, seleção e composição.
...
comment on function public.storefront_product(text,uuid) is
  'Projeção pública mínima do produto SHARK. configuration e metadata permanecem server-side e não são expostos ao browser.';
