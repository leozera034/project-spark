...
-- ============================================================
--
-- Esta fase reutiliza a matriz de autorização já existente. app_permission é
-- um ENUM, não uma tabela de catálogo. Relatórios operacionais da loja usam
-- reports.view_operational; o entregador usa courier.view_self.

-- 1. Fonte privada de fatos de entrega concluída --------------
...
SELECT
  ...
  (d.completed_at AT TIME ZONE s.timezone)::date AS completed_local_date
...
-- 2. Índices de performance para relatórios -------------------
...
-- 3. Função auxiliar: calcular limites de período -------------
...
BEGIN
  ...
  _today_local := (now() AT TIME ZONE _tz)::date;

  CASE _period_type
    WHEN 'today' THEN
      start_utc := (_today_local::timestamp AT TIME ZONE _tz);
      end_utc := start_utc + interval '1 day';
    WHEN 'week' THEN
      start_utc := (date_trunc('week', _today_local::timestamp) AT TIME ZONE _tz);
      end_utc := start_utc + interval '7 days';
    WHEN 'month' THEN
      start_utc := (date_trunc('month', _today_local::timestamp) AT TIME ZONE _tz);
    ...
    WHEN 'custom' THEN
      ...
      END IF;
      start_utc := (_start_date::timestamp AT TIME ZONE _tz);
      end_utc := ((_end_date + 1)::timestamp AT TIME ZONE _tz);
    ELSE
...
REVOKE ALL ON FUNCTION private.get_report_period_limits(uuid,text,date,date) FROM PUBLIC;

-- 4. RPC: Resumo de entregas da loja --------------------------
...
BEGIN
  ...
  SELECT * INTO _limits
    FROM private.get_report_period_limits(_sid, _period_type, _start_date, _end_date);
...
-- 5. RPC: Série diária de entregas ----------------------------
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(d ORDER BY d.local_date), '[]'::jsonb)
    INTO _series
    FROM (
      SELECT completed_local_date::text AS local_date,
...
-- 6. RPC: Comparação operacional entre entregadores -----------
...
BEGIN
  ...
  SELECT coalesce(jsonb_agg(r ORDER BY r.completed_deliveries DESC, r.courier_name), '[]'::jsonb)
    INTO _rows
    FROM (
      SELECT courier_id, courier_name, courier_status,
             count(*) AS completed_deliveries
        FROM private.completed_delivery_facts
       WHERE store_id = _sid
         AND completed_at >= _limits.start_utc
...
-- 7. RPC: Histórico paginado de entregas concluídas -----------
...
-- 8. RPC: Contador próprio do entregador ----------------------
...
BEGIN
  SELECT courier_id, store_id INTO _cid, _sid
    FROM private.require_current_courier();

  IF NOT private.has_permission('courier.view_self', _sid) THEN
  ...
  SELECT count(*) INTO _today_count
    FROM private.completed_delivery_facts
   WHERE courier_id = _cid
     AND completed_at >= _today_limits.start_utc
     AND completed_at < _today_limits.end_utc;
  SELECT count(*) INTO _week_count
    FROM private.completed_delivery_facts
   WHERE courier_id = _cid
     AND completed_at >= _week_limits.start_utc
     AND completed_at < _week_limits.end_utc;
  SELECT count(*) INTO _month_count
    FROM private.completed_delivery_facts
   WHERE courier_id = _cid
     AND completed_at >= _month_limits.start_utc
     AND completed_at < _month_limits.end_utc;

  IF _start_date IS NOT NULL OR _end_date IS NOT NULL THEN
    IF _start_date IS NULL OR _end_date IS NULL THEN
      RAISE EXCEPTION 'START_AND_END_DATE_REQUIRED' USING ERRCODE='P0001';
    END IF;
    SELECT * INTO _custom_limits
      FROM private.get_report_period_limits(_sid, 'custom', _start_date, _end_date);
    SELECT count(*) INTO _custom_count
      FROM private.completed_delivery_facts
     WHERE courier_id = _cid
       AND completed_at >= _custom_limits.start_utc
       AND completed_at < _custom_limits.end_utc;
  END IF;
...
-- 9. RPC: Histórico próprio do entregador --------------------
