-- SHARK — aplica min/max/incluídos por tamanho no motor canônico v2.
...
begin
  ...
  for _link in
    select
      ...
      og.role,
      og.configuration,
      coalesce(vr.min_selections,pog.min_selections,og.min_selections) effective_min,
      coalesce(vr.max_selections,pog.max_selections,og.max_selections) effective_max,
      coalesce(vr.included_selections,og.included_selections) effective_included
    from public.product_option_groups pog
    join public.option_groups og on og.id=pog.option_group_id and og.store_id=pog.store_id
    left join public.product_variant_option_group_rules vr
      on vr.store_id=pog.store_id
     and vr.product_id=pog.product_id
     and vr.option_group_id=pog.option_group_id
     and vr.product_variant_id=_v.id
  ...
  loop
    ...
    _charge_num:=0; _charge_den:=0; _charge_max:=null; _seen:=array[]::uuid[]; _free_left:=coalesce(_link.effective_included,0); _group_value:=0;
    ...
    if _link.selection_type='quantidade' then
      if _sum_qty<_link.effective_min and (_link.is_required or _sum_qty>0) then _errors:=_errors||'SELECTION_BELOW_MINIMUM'; end if;
      if _sum_qty>_link.effective_max then _errors:=_errors||'SELECTION_ABOVE_MAXIMUM'; end if;
    else
      if _count_items<_link.effective_min and (_link.is_required or _count_items>0) then _errors:=_errors||'SELECTION_BELOW_MINIMUM'; end if;
      if _count_items>_link.effective_max then _errors:=_errors||'SELECTION_ABOVE_MAXIMUM'; end if;
    end if;
    ...
    _breakdown:=_breakdown||jsonb_build_object(
      ...
      'price_effect',_effective_effect,
      'included_selections',_link.effective_included,
      'min_selections',_link.effective_min,
      'max_selections',_link.effective_max,
