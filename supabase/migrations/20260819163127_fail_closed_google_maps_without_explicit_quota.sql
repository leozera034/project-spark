...
declare
  ...
  _paid_maps_metric boolean := lower(coalesce(_provider,'')) = 'google_maps'
    and _feature_code = 'delivery.smart'
    and _metric_code in ('routes.compute','geocoding.address');
begin
  ...
  if not found then
    return not _paid_maps_metric;
  end if;

  if _paid_maps_metric and _limit.limit_action = 'block' and _limit.hard_limit_units is null then
    return false;
  end if;

  if _limit.hard_limit_units is null or _limit.limit_action <> 'block' then
    return true;
  end if;
