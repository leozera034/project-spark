...
begin
  ...
  _event_code := case new.status::text
    ...
    when 'aguardando_retirada' then 'pedido_aguardando_retirada' when 'entregue' then 'pedido_entregue'
    when 'retirado' then 'pedido_retirado' when 'recusado' then 'pedido_recusado' when 'cancelado' then 'pedido_cancelado' else null end;
