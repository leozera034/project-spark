-- SHARK — remove a compatibilidade temporária baseada em role falsa.
-- A experiência pública agora deve usar capabilities.burger_experience / meal_experience.
-- Restauramos Acompanhamento como role=side para manter semântica de domínio correta.
...
where og.id=pog.option_group_id and og.store_id=pog.store_id
  ...
  and og.role='addon'
  and og.configuration->>'experience'='burger'
  ...
  loop
    ...
    if _name is not null then
      ...
      set name=_name,sort_order=_order,
          ...
          configuration=configuration||jsonb_build_object('experience','burger','semantic_role',_g.role,'experience_order',_order),updated_at=now()
