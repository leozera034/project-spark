...
async function stripe(path:string,account:string,data:URLSearchParams,idempotencyKey:string){const sk=Deno.env.get("STRIPE_SECRET_KEY")?.trim();if(!sk)return{ok:false,status:503,body:{} as J};const h=new Headers({Authorization:`Bearer ${sk}`,"Stripe-Account":account,"content-type":"application/x-www-form-urlencoded","Idempotency-Key":idempotencyKey});const c=new AbortController(),t=setTimeout(()=>c.abort(),TIMEOUT_MS);try{const r=await fetch(`${STRIPE_API}${path}`,{method:"POST",headers:h,body:data.toString(),signal:c.signal});return{ok:r.ok,status:r.status,body:obj(await r.json().catch(()=>({})))}}finally{clearTimeout(t)}}
...
Deno.serve(async(req:Request)=>{
  ...
  const account=str(c.stripe_account_id),amount=int(c.amount_cents),fee=int(c.application_fee_amount)??0,currency=str(c.currency)??"brl",orderNumber=str(c.order_number)??"";
  if(!account||!amount||amount<=0)return json(req,{ok:false,error:"payment_context_invalid"},409);

  const success=`${APP_ORIGIN}/loja/${encodeURIComponent(str(body?.slug)??"")}/pedido-enviado?payment=success`;
  const cancel=`${APP_ORIGIN}/loja/${encodeURIComponent(str(body?.slug)??"")}/pedido-enviado?payment=cancelled`;
  const data=form({
    mode:"payment",
    success_url:success,
    cancel_url:cancel,
  ...
  if(fee>0)data.set("payment_intent_data[application_fee_amount]",String(fee));
  const up=await stripe("/v1/checkout/sessions",account,data,`comandiva-order-checkout-${orderId}`);
  ...
  return json(req,{ok:true,checkoutUrl:url,checkoutSessionId:sessionId});
});
