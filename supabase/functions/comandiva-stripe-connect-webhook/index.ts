...
Deno.serve(async(req:Request)=>{
  ...
  try{
    const resource=obj(obj(event.data).object);
    ...
    if(eventType==="charge.refunded"){const ok=await syncRefund(resource,eventId,connectedAccount,eventCreated);await finish(ok?"processed":"ignored",ok?null:"refund_not_mapped");return response(200,{ok:true,processed:ok})}
    if(eventType==="charge.dispute.created"||eventType==="charge.dispute.updated"||eventType==="charge.dispute.closed"){const ok=await syncDispute(resource,eventId,connectedAccount,eventCreated);await finish(ok?"processed":"ignored",ok?null:"dispute_not_mapped");return response(200,{ok:true,processed:ok})}
