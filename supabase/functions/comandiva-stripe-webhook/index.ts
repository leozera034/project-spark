...
async function validSignature(raw:string,header:string,secret:string){let ts="";const signatures:string[]=[];for(const part of header.split(",")){const i=part.indexOf("=");if(i<0)continue;const k=part.slice(0,i).trim(),v=part.slice(i+1).trim();if(k==="t")ts=v;if(k==="v1")signatures.push(v)}if(!ts||!signatures.length||!/^\d+$/.test(ts))return false;const drift=Math.abs(Date.now()/1000-Number(ts));if(!Number.isFinite(drift)||drift>TOLERANCE_SECONDS)return false;const computed=await hmac(secret,`${ts}.${raw}`);return signatures.some(s=>safeEq(s,computed))}
...
async function reconcileSubscription(raw:J,eventCreated:number|null){
  ...
  const periodStart=unixIso(raw.current_period_start)??unixIso(first.current_period_start);
  const periodEnd=unixIso(raw.current_period_end)??unixIso(first.current_period_end);
  ...
  if(ext.startsWith("comandiva:addon:")){
    const {error}=await a.rpc("billing_reconcile_stripe_subscription",{_provider_subscription_id:sid,_provider_status:status,_external_reference:ext,_provider_price_id:priceId,_current_period_start:periodStart,_current_period_end:periodEnd,_cancel_at_period_end:raw.cancel_at_period_end===true} as never);
    return{relevant:true,kind:"addon",ok:!error,error:error?.message};
  }
