...
async function consumeRateLimit(admin: ReturnType<typeof adminClient>) {
  const { data, error } = await admin.rpc("consume_edge_rate_limit", {
    _key: "smart-delivery:ors:admin-geocode:hour",
    _limit: 20,
...
Deno.serve(async (req: Request) => {
  ...
  const admin = adminClient();
  if (!(await consumeRateLimit(admin))) return json({ ok: false, error: "rate_limited" }, 429);
  ...
  if (action !== "preview_store_geocode") return json({ ok: false, error: "action_not_allowed" }, 403);
